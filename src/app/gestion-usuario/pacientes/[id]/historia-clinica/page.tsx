...
export default function HistoriaClinicaPage() {
  ...
  useEffect(() => {
    if (paciente) {
      ...
      const budgetsForPatient = mockPresupuestosData.filter(
        (p) =>
      ...
      budgetsForPatient.forEach((budget) => {
        const doctor = mockPersonalData.find(
          (d) => d.id === budget.doctorResponsableId
        );
  ...
  const filteredAndSortedHistory = useMemo(() => {
    ...
    return filtered;
  }, [clinicalHistory, doctorFilter, sortConfig]);
  ...
  if (loading)
    return (
      <div className="flex justify-center items-center h-screen">
        <p>Cargando datos de historia clínica...</p>
      </div>
    );
  if (!paciente || !persona) {
    return (
      ...
      <div className="flex-1">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Resumen de los tratamientos y procedimientos realizados al
              paciente.
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    {columnVisibility.tratamiento && (
                      <TableHead>Tratamiento</TableHead>
                    )}
                    {columnVisibility.notas && <TableHead>Notas</TableHead>}
                    {columnVisibility.honorarios && (
                    ...
                    filteredAndSortedHistory.map((item, index) => (
                      <TableRow key={index}>
                        ...
                        {columnVisibility.doctor && (
                          <TableCell>
                            {item.doctor
                              ? `${item.doctor.persona.nombre} ${item.doctor.persona.apellidoPaterno}`
                              : "N/A"}
                          </TableCell>
                        )}
                        {columnVisibility.tratamiento && (
