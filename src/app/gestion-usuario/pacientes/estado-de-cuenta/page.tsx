...
export default function EstadoDeCuentaPage() {
  ...
  const refreshBudgets = async () => {
    try {
      ...
      toast({
        title: "Error",
        description: "No se pudieron actualizar los presupuestos.",
        variant: "destructive",
      });
    }
  };
  ...
  useEffect(() => {
    const fetchPageData = async () => {
      ...
      if (foundPaciente) {
        ...
        const foundHistoriaClinica = mockHistoriasClinicasData.find(
          (hc) => hc.id === foundPaciente.idHistoriaClinica
        );
      ...
      try {
        ...
        const filteredPagos = mockPagosData
          .filter((p) =>
            p.itemsPagados.some((ip) =>
              presupuestoIds.includes(ip.idPresupuesto)
            )
          )
          .sort(
            (a, b) =>
