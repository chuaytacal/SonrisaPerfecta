  ...
  return (
    <Card className="mb-6">
      <CardContent className="p-4 grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4 items-start">
          ...
          <Dialog open={isTagModalOpen} onOpenChange={setIsTagModalOpen}>
            <DialogTrigger asChild>
              <Button
                variant="ghost"
                size="sm"
                className="text-primary hover:text-primary hover:bg-primary/10 p-1 h-auto mt-1 rounded-sm"
              >
            ...
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
                ...
                <DialogDescription>
                  Seleccione una etiqueta para agregar al paciente.
                </DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                <Select
                  ...
                  onValueChange={(value) =>
                    setSelectedTagToAdd(value as EtiquetaPaciente)
                  }
                >
                  <SelectTrigger>
        ...
        <div className="space-y-2">
          <Label
            htmlFor={`notas-paciente-${patientId}`}
          ...
          {isEditingNotes ? (
            <div className="space-y-2">
              <Textarea
                id={`notas-paciente-edit-${patientId}`}
              ...
              <div className="flex justify-end gap-2">
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={handleInternalCancelEditNotes}
                >
                  <X className="mr-1 h-3 w-3" />
                  Cancelar
                </Button>
                <Button size="sm" onClick={handleInternalSaveNotes}>
            ...
            <Card
              onClick={() => {
                setEditingNotesText(
                  notas === "Sin notas registradas." ? "" : notas
                );
                setIsEditingNotes(true);
              }}
              className="p-3 min-h-[80px] cursor-pointer hover:bg-muted/50"
            >
          ...
          <div>
            ...
            {enfermedades && enfermedades.length > 0 ? (
              enfermedades.map((enf) => (
                <Badge
                  key={`${patientId}-enf-${enf}`}
                  variant="outline"
                  className="border-orange-500 text-orange-600 mr-1 mb-1"
                >
                  {enf}
                </Badge>
              ))
          ...
          <div className="mt-2">
            ...
            {alergias && alergias.length > 0 ? (
              alergias.map((alergia) => (
                <Badge
                  key={`${patientId}-alergia-${alergia}`}
                  variant="outline"
                  className="border-red-500 text-red-600 mr-1 mb-1"
                >
                  {alergia}
                </Badge>
              ))
