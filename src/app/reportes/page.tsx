...
export default function ReportesPage() {
  ...
  const { kpis, ingresosPorDia, distribucionServicios, rendimientoPorDoctor } =
    useMemo(() => {
      const from = dateRange?.from ? startOfDay(dateRange.from) : undefined;
      const to = dateRange?.to ? endOfDay(dateRange.to) : undefined;

      // --- Global KPIs (not affected by filters) ---
      const presupuestosActivosGlobal = mockPresupuestosData.filter(
        (p) => p.estado === "Creado" || p.estado === "Pagado"
      );
      const totalPresupuestadoGlobal = presupuestosActivosGlobal.reduce(
        (acc, presupuesto) => {
      ...
      const totalPagadoGlobal = presupuestosActivosGlobal.reduce(
        (acc, presupuesto) => acc + presupuesto.montoPagado,
        0
      );
      ...
      if (!from || !to) {
        return {
          kpis: {
            ...
            nuevosPacientes: 0,
            servicioPopular: "N/A",
            totalPagadoHistorico: totalPagadoGlobal,
            saldoPendienteTotal: saldoPendienteGlobal,
          },
          ingresosPorDia: [],
      ...
      const pagosEnRango: Pago[] = mockPagosData.filter((p) => {
        ...
        const doctorMatch =
          doctorFilter === "all" || p.doctorResponsableId === doctorFilter;
        return (
          isWithinInterval(fechaPago, { start: from, end: to }) &&
          doctorMatch &&
          p.estado === "activo"
        );
      });
      ...
      const citasEnRango: Appointment[] = mockAppointmentsData.filter((c) => {
        ...
        const doctorMatch =
          doctorFilter === "all" || c.idDoctor === doctorFilter;
        const statusMatch = statusFilter === "all" || c.estado === statusFilter;
        return (
          isWithinInterval(fechaCita, { start: from, end: to }) &&
          doctorMatch &&
          statusMatch
        );
      });
      ...
      // KPI Calculations
      const totalIngresos = pagosEnRango.reduce(
      ...
      const ingresosPorDiaArray = Object.entries(ingresosPorDiaData)
        ...
        .map((item) => ({
          ...item,
          fecha: format(new Date(item.fecha + "T00:00:00"), "dd MMM", {
            locale: es,
          }),
        }));

      // Filter budgets for service distribution
      const presupuestosEnRango: Presupuesto[] = mockPresupuestosData.filter(
        (pr) => {
          const fechaPresupuesto = new Date(pr.fechaAtencion); // Use attention date for relevance
          const doctorMatch =
            doctorFilter === "all" || pr.doctorResponsableId === doctorFilter;
          return (
            isWithinInterval(fechaPresupuesto, { start: from, end: to }) &&
            doctorMatch &&
            (pr.estado === "Creado" || pr.estado === "Pagado")
          );
        }
      );

      // Distribucion de Servicios (now based on quantity)
      const serviciosContador = presupuestosEnRango
        .flatMap((p) => p.items)
        .reduce((acc, item) => {
          acc[item.procedimiento.denominacion] =
            (acc[item.procedimiento.denominacion] || 0) + item.cantidad;
          return acc;
        }, {} as Record<string, number>);

      const distribucionServiciosArray = Object.entries(serviciosContador)
        .map(([name, value]) => ({
          name,
          value,
        }))
      ...
      const servicioPopular =
        distribucionServiciosArray.length > 0
          ? distribucionServiciosArray[0].name
          : "N/A";

      // Rendimiento por Doctor
      const rendimientoData = mockPersonalData
        .map((doctor) => {
          const pagosDoctor = mockPagosData.filter(
            (p) =>
              p.doctorResponsableId === doctor.id &&
              p.estado === "activo" &&
              isWithinInterval(new Date(p.fechaPago), { start: from, end: to })
          );
          const citasDoctor = mockAppointmentsData.filter(
            (c) =>
              c.idDoctor === doctor.id &&
              c.estado === "Atendido" &&
              isWithinInterval(new Date(c.start), { start: from, end: to })
          );
    ...
    <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              S/ {kpis.totalIngresos.toFixed(2)}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div
              ...
              title={kpis.servicioPopular}
            >
              {kpis.servicioPopular}
            </div>
            <p className="text-xs text-muted-foreground">
              En el período seleccionado
            </p>
        ...
        <Card className="md:col-span-2 lg:col-span-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              S/ {kpis.totalPagadoHistorico.toFixed(2)}
            </div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card className="md:col-span-2 lg:col-span-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">
              S/ {kpis.saldoPendienteTotal.toFixed(2)}
            </div>
            <p className="text-xs text-muted-foreground">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Resumen de ingresos diarios en el período seleccionado.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isClient ? (
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Ingresos y citas atendidas por cada doctor en el período
            seleccionado.
          </CardDescription>
        </CardHeader>
        <CardContent>
              ...
              {rendimientoPorDoctor.length === 0 && (
                <TableRow>
                  <TableCell colSpan={3} className="text-center h-24">
                    No hay datos de rendimiento para el período y doctor
                    seleccionados.
                  </TableCell>
                </TableRow>
              )}
