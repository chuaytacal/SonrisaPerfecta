...
const TreatmentPlanTable: React.FC<Props> = ({ dientesMap, odontogramType, onOdontogramDataChange, onUpdateDientesMap }) => {
  ...
  const [modalServicio, setModalServicio] = useState<{ index: number; item: OdontogramDataItem } | null>(null);
  ...
  const procesarDatos = (currentDientesMap: DientesMap): OdontogramDataItem[] => {
    ...
    for (const [dienteStr, hallazgos] of Object.entries(currentDientesMap)) {
      ...
      if (odontogramType === 'Primary' && !isPrimaryTooth(numeroDiente)) continue;
  ...
  const guardarNota = (index: number) => {
    setPlanTratamiento(prev => prev.map((item, i) => i === index ? { ...item, nota: inputNota } : item));
    cerrarModalNotas();
  };
  ...
  const guardarServicios = () => {
    ...
    const dienteIds = Array.isArray(modalServicio.item.diente) ? modalServicio.item.diente : [modalServicio.item.diente];
  ...
  useEffect(() => {
    ...
    if (editandoIndex !== null) document.addEventListener('mousedown', handleClickOutside);
  ...
  const getAbreviaturas = (hallazgo: Omit<Hallazgo, 'grupo' | 'servicios'>): { abreviatura: string, color: string }[] => {
    ...
    const fromCaras = hallazgo.cara ? Object.values(hallazgo.cara).filter(c => c.detalle).map(c => ({ abreviatura: c.detalle!.abreviatura, color: c.color || hallazgo.color })) : [];
  ...
  const filteredProcedimientoOptions = procedimientoOptions.filter(opt => !serviciosEnModal.some(s => s.id === opt.value));
  ...
  <div className="relative overflow-x-auto shadow-md rounded-lg">
              ...
              <tr key={index} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center">
                    <div className="h-3 w-3 rounded-full mr-2" style={{ backgroundColor: item.hallazgo.color }} />
                    <div>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {item.servicios && item.servicios.length > 0 ? (
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <Button variant="ghost" size="icon" className="h-6 w-6 shrink-0 text-primary hover:bg-primary hover:text-primary-foreground" onClick={() => abrirModalServicios(index, item)}>
                        <PlusCircle className="h-4 w-4" />
                      </Button>
                    </div>
                  ) : (
                    <Button variant="ghost" className="text-muted-foreground hover:bg-primary hover:text-primary-foreground px-2" onClick={() => abrirModalServicios(index, item)}>
                ...
                <td className="px-6 py-4 whitespace-nowrap relative">
                  <div className="flex flex-wrap items-center gap-1">
                    {abreviaturas.map((detalle, i) => (
                      <span key={i} className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${detalle.color === '#E40000' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'}`}>
                        {detalle.abreviatura}
                      </span>
                    ))}
                    {item.nota && (
                      <span title={item.nota} className="inline-flex items-center max-w-[200px] truncate px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                        {item.nota}
                      </span>
                    )}
                    <button
    ...
    {editandoIndex !== null && modalPos && (
      <div ref={modalRef} className="fixed bg-white border border-gray-300 rounded-md shadow-lg p-2 z-50" style={{ top: modalPos.top, left: modalPos.left }}>
        <textarea
          value={inputNota}
        ...
        <div className="flex justify-end">
          <button onClick={() => guardarNota(editandoIndex)} className="text-xs px-3 py-0.5 bg-blue-500 text-white rounded hover:bg-blue-600">
            Guardar
          </button>
        </div>
      </div>
            ...
            <div className="px-6 py-4 space-y-4">
              <div className="w-full mx-auto mb-4 px-4 py-3 rounded-lg bg-muted/50 border">
                <div className="flex justify-center items-center gap-3">
                  ...
                  <div className="w-5 h-5 rounded-full border border-gray-300 shadow-inner shrink-0" style={{ backgroundColor: modalServicio.item.hallazgo.color }} title="Color del hallazgo" />
                </div>
              </div>
