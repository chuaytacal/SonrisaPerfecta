...
const statusConfig: Record<EstadoPresupuesto, { label: string; icon: React.ElementType, className: string, color: string }> = {
  Creado: { label: 'Creado', icon: FileText, className: 'text-blue-600', color: '#3b82f6' },
  Aceptado: { label: 'Aceptado', icon: ThumbsUp, className: 'text-green-600', color: '#16a34a' },
  Rechazado: { label: 'Rechazado', icon: ThumbsDown, className: 'text-red-600', color: '#dc2626' },
  Abandonado: { label: 'Abandonado', icon: HeartOff, className: 'text-gray-500', color: '#6b7280' },
  Terminado: { label: 'Terminado', icon: CheckCircle2, className: 'text-purple-600', color: '#9333ea' },
  Otro: { label: 'Otro', icon: Circle, className: 'text-gray-500', color: '#6b7280' },
};

export function BudgetCard({ presupuesto: initialPresupuesto }: BudgetCardProps) {
  ...
  const handleStateChange = (newState: EstadoPresupuesto) => {
    // In a real app, this would be an API call. Here, we update mock data.
          ...
          <div className="flex items-center gap-4">
              <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                       <Button variant="outline" className={cn("text-xs font-semibold h-7 px-2 gap-1", statusConfig[estado].className)} style={{ borderColor: statusConfig[estado].color }}>
                  ...
                  <DropdownMenuContent align="start">
                    ...
                    {Object.entries(statusConfig).map(([key, config]) => (
                      <DropdownMenuItem key={key} onClick={() => handleStateChange(key as EstadoPresupuesto)}>
                        <config.icon className={cn("mr-2 h-4 w-4", config.className)} />
